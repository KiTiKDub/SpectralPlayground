...
FFTProcessor::FFTProcessor(int order, int overlap) : fft(order),
...
void FFTProcessor::prepareToPlay(int order, int overlap)
{
...
void FFTProcessor::processBlock(float *data, int numSamples, bool bypassed)
{
    ...
    {
        data[i] = processSample(data[i], bypassed, factor);
    }
}

float FFTProcessor::processSample(float sample, bool bypassed, float factor )
{
    ...
    {
        count = 0;
        processFrame(bypassed, factor);
    }

    return outputSample;
}

void FFTProcessor::processFrame(bool bypassed, float factor)
{
...
void FFTProcessor::processSpectrum(float *data, int numBins, float factor)
{
    ...
    for (int i = 0; i < numBins; ++i)
    {
        ...
        auto compresionValue = 

        // auto crusher = pow(2, 8);
        // auto crushedData = floor(crusher * phase) / crusher;
